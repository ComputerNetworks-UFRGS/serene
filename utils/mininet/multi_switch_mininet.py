...
parser.add_argument(
    "--auto-control-plane",
    help="enable automatic control plane population",
    action="store_true",
)
parser.add_argument(
...
parser.add_argument(
    "--target",
    "-t",
    help="Target in manifest file to run",
    type=str,
    action="store",
    required=True,
)
parser.add_argument(
    "--log-dir",
    "-l",
    help="Location to save output to",
    type=str,
    action="store",
    required=True,
)
parser.add_argument(
    "--cli-message",
    help="Message to print before starting CLI",
    type=str,
    action="store",
    required=False,
    default=False,
...
def main():
    ...
    latencies = dict(
        [("".join(sorted(l[:2])), l[2]) for l in conf["links"] if len(l) >= 3]
    )
